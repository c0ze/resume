...
let renderInline = toks =>
  ...
  ->Array.mapWithIndex((tok, i) => {
    ...
    switch tKind(tok) {
    | "bold" => <strong key className="font-bold"> {React.string(tVal(tok))} </strong>
    | "italic" => <em key> {React.string(tVal(tok))} </em>
    | "code" =>
      <code key className="md-code"> {React.string(tVal(tok))} </code>
    | "link" =>
      tHref(tok)->safeHref
        ? <a key href={tHref(tok)} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2">
